...
This library is a client for the Horusync server, so you need to have a Horusync server running to be able to use this library.
To see how to install the server using PHP + Laravel go to  [Horusync PHP](https://github.com/apptanksas/horus-sync-php).
...
```kotlin class MainApplication : Application() {  
 override fun onCreate() { super.onCreate() setupHorus() }  
 private fun setupHorus() {  
 val BASE_SERVER_URL = "https://api.yourdomain.com/sync"  
 // Configure Horus        HorusConfigurator(BASE_SERVER_URL, isDebug = true).configure(this)  
  
 // Register the activity lifecycle callbacks        registerActivityLifecycleCallbacks(HorusActivityLifeCycle())  
 }} ```   
## IOS  
...
```kotlin HorusDataFacade.onReady {  
 /** PUT YOUR CODE **/} ``` ### Subscribe to data changes  
...
```kotlin HorusDataFacade.addDataChangeListener(object : DataChangeListener {  
  
 override fun onInsert(entity: String, id: String, data: DataMap) { /** WHEN IS INSERTED A NEW RECORD **/ } override fun onUpdate(entity: String, id: String, data: DataMap) { /** WHEN IS UPDATED A RECORD **/ }  
 override fun onDelete(entity: String, id: String) { /** WHEN IS DELETED A RECORD **/ }}) ```   
Remove the listener  
...
* The attributes sync_created_at and sync_updated_at are automatically managed by Horus and always  
  contain the date and time of the record creation and last update respectively with timezone UTC.
...
when (result) {  
 is DataResult.Success -> { val entityId = result.data /** YOUR CODE HERE WHEN INSERT IS SUCCESSFUL */ } is DataResult.Failure -> { /** YOUR CODE HERE WHEN INSERT FAILS */ } is DataResult.NotAuthorized -> { /** YOUR CODE HERE WHEN INSERT FAILS BECAUSE OF NOT AUTHORIZED */ }} ```   
Alternative result validation  
  
```kotlin result.fold(  
 onSuccess = { val entityId = result.data /** YOUR CODE HERE WHEN INSERT IS SUCCESSFUL */ }, onFailure = { /** YOUR CODE HERE WHEN INSERT FAILS */ }) ```   
...
```kotlin val userId = "0ca2caa1-74f1-4e58-a6a7-29e79efedfe4"  
val newName = "Elton"  
val result = HorusDataFacade.update(  
 "users", userId, mapOf( "name" to newName ))  
when (result) {  
 is DataResult.Success -> { /** YOUR CODE HERE WHEN SUCCESS */ }  
 is DataResult.Failure -> { /** YOUR CODE HERE WHEN FAILURE */ }  
 is DataResult.NotAuthorized -> { /** YOUR CODE HERE WHEN UPDATE FAILS BECAUSE OF NOT AUTHORIZED */ }} ```   
### Delete a record  
...
when (result) {  
 is DataResult.Success -> { /** YOUR CODE HERE WHEN SUCCESS */ } is DataResult.Failure -> { /** YOUR CODE HERE WHEN FAILURE */ } is DataResult.NotAuthorized -> { /** YOUR CODE HERE WHEN UPDATE FAILS BECAUSE OF NOT AUTHORIZED */ }} ```   
## Simple record query  
...
```kotlin val whereConditions = listOf(  
...
HorusDataFacade.querySimple("users", whereConditions, orderBy = "name")  
 .fold( onSuccess = { users -> //** YOUR CODE HERE WHEN SUCCESS */      },   
      onFailure = {  
...
```kotlin HorusAuthentication.setupUserAccessToken("{ACCESS_TOKEN}") ```
### Clear session
...
```kotlin HorusAuthentication.setUserActingAs("{USER_OWNER_ID}") ```   
# 3. Local testing  
