...
sealed class LocalDatabase {
     ...
     */
    open class Operation(open val table: String)
    ...
    data class UpdateRecord(
        ...
        val operator: OperatorComparator = OperatorComparator.AND
    ) : Operation(table)
    ...
    data class OperatorKey(
        val operator: OperatorComparator,
...
fun EntityAttribute<*>.toDBColumnValue(): LocalDatabase.ColumnValue {
    return LocalDatabase.ColumnValue(name, value!!)
}

fun List<EntityAttribute<*>>.mapToDBColumValue(): List<LocalDatabase.ColumnValue> {
